...
| `/community` | **The Forge** — finalized name, replacing the old "Founding 100" framing. Explains honestly why Afri3B isn't inside the WhatsApp groups (Meta's official API can't post into groups), links to messaging Afri3B one-on-one instead, and to joining The Forge itself. The marketing agent lives as a WhatsApp command now (see `BACKEND_REQUIREMENTS.md` §9), not a website page. |
